...
###############################################################################
# Thesis pipeline: v6 tons-only L proxy + v5 tons-only raw LP + fixed v3 Model 1A
###############################################################################
...
LP_DIR="Data/LP"
LP_BUILD_DIR="$LP_DIR/raw_from_l_v5_tonsonly"
...
LP_RAW="$LP_DIR/Build_LP_Panel_raw_from_L_v5_tonsonly.py"

M1A="$DESIGN_DIR/Model_1A_v7_quarterly_windows_fixed_v3.py"
M1A_LP_INPUT="$LP_BUILD_DIR/LP_Panel.tsv"
...
###############################################################################
# 0) Check required code files
###############################################################################

stage "0) Check required code files"
...
# Model 2 is intentionally not run here, because it still needs rewiring to the
# v7 Model 1A output folder before it can be trusted.
...
stage "1b) Promote labor helper outputs"
...
###############################################################################
# 2) LP build: v5 raw throughput over new L
###############################################################################

stage "2) Build LP branch: raw_from_l_v5_tonsonly"
...
stage "2b) Promote LP component outputs"
...
echo "Model 2 is intentionally skipped until it is rewired to the v7 Model 1A outputs."
