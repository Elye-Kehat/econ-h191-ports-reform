...
def infer_thesis_root(start: Path) -> Path:
    here = start.resolve()
    for p in [here] + list(here.parents):
        if (p / "Data").exists():
            return p
    raise RuntimeError("Could not locate thesis root.")


def main():
    ap = argparse.ArgumentParser()
    ap.add_argument("--k-wide", dest="k_wide", default="Data/K/Interpolation Output/interpolation_02_monthly_entity_series_wide.tsv")
    ap.add_argument("--lproxy", default="Data/L_proxy/L_Proxy.tsv")
    ap.add_argument("--out-dir", dest="out_dir", default="Data/KL")
    args = ap.parse_args()

    thesis_root = infer_thesis_root(Path(__file__))

    k_path = thesis_root / args.k_wide
    l_path = thesis_root / args.lproxy
    kl_dir = thesis_root / args.out_dir
    ...
    manifest = {
        ...
        "out_dir": str(kl_dir),
        "series_mapping": {
            "LEGACY": {"K": "K_HPC_kNIS", "L": "Haifa-Legacy"},
            "ENTRANT": {"K": "K_SIPG_kNIS", "L": "Haifa-Bayport"},
            "HAIFA_TOTAL": {"K": "K_HAIFA_TOTAL_kNIS", "L": "Haifa-Legacy + Haifa-Bayport"},
        },
        "outputs": [
    ...
    print("Done.")
    print(f"K source: {k_path}")
    print(f"L source: {l_path}")
