#!/usr/bin/env python3
"""
Build_KL_Panel_new_v2.py

Build a clean monthly K/L panel from the new interpolation_02 capital outputs
and a user-specified labor proxy.

Why this revision exists
------------------------
The original script hard-coded:
    Data/L_proxy/L_Proxy.tsv

This version adds a --lproxy argument so the same K/L build can be run against:
- the legacy labor proxy
- common_rule_v5
- any later labor-proxy implementation

The default behavior is unchanged if no arguments are supplied.
"""
...
def load_l_proxy(path: Path) -> pd.DataFrame:
    ...
    require_columns(df, ["port", "terminal", "year", "month", "L_hours_i_m"], "L_Proxy")
...
def main() -> None:
    ...
    ap.add_argument("--k-long", dest="k_long", default="Data/K/Interpolation Output/interpolation_02_monthly_entity_series_long.tsv")
    ap.add_argument("--out-dir", dest="out_dir", default="Data/KL")
    args = ap.parse_args()

    thesis_root = find_thesis_root(Path(__file__))

    l_proxy_path = thesis_root / args.lproxy
    k_long_path = thesis_root / args.k_long
    out_dir = thesis_root / args.out_dir
    ...
    manifest = {
        "builder": "Build_KL_Panel_new_v2.py",
        "design_note": (
            "Uses interpolation_02 productive capital outputs directly and builds only the three core "
            "Haifa analysis series. Adds configurable labor-proxy input so the same builder can be "
            "run against old L_Proxy or common_rule implementations."
        ),
        "inputs": {
