...
"""
Build_L_Proxy_CommonRule_v4.py
==============================

Quarter-flat version of the common-rule labor proxy.

Main idea
---------
1) Keep the annual labor-hours anchor:
       H_{i,y} = TEU_{i,y} / Pi^{work}_{i,y}

2) Build a quarterly activity proxy:
       activity_{i,q} = share_{i,p,q} * tons_{p,q}

3) Allocate the annual anchor across quarters:
       w_{i,q} = activity_{i,q} / sum_{r in y(q)} activity_{i,r}
       L_{i,q} = H_{i,y(q)} * w_{i,q}

4) Monthlyize the quarter in a flat way:
   - in ordinary quarters:
         L_{i,m} = L_{i,q} / 3
   - in opening quarters:
         pre-opening months stay at 0
         the quarter total is split evenly across active months only

Why this version exists
-----------------------
The earlier monthly common-rule versions still produced terminal-month paths that
were too jagged and too obviously driven by interpolation choices.

This version is deliberately more conservative:
- quarterly truth
- flat within quarter
- minimal arbitrariness
- still monthly on disk so the rest of the pipeline can read it

Why this may be better
----------------------
- Much less noisy than using month-specific terminal residual tons
- Much less noisy than using month-specific quarter-share * port tons
- Better aligned with the fact that the true terminal output information is
  quarterly post-reform, not monthly
- Still avoids terminal-specific hyperparameters

Known limitation
----------------
This is intentionally piecewise-flat within quarter. That is less realistic than
a truly observed monthly labor path, but it is more honest and stable than the
earlier monthlyized versions.
...
Primary outputs
---------------
    labor_hours_monthly_terminal_commonrule_v4.tsv
    labor_hours_monthly_port_commonrule_v4.tsv
    labor_anchor_terminal_year_commonrule_v4.tsv
    L_Proxy_commonrule_v4.tsv
    lproxy_commonrule_v4_qa.tsv
    lproxy_commonrule_v4_old_new_anchor_compare.tsv
    _meta_l_proxy_commonrule_v4.json
"""
...
OPEN_MONTH_BY_TERMINAL = {
    "Haifa-Bayport": 202109,
    "Ashdod-HCT": 202211,
}
...
def load_teu_panel(teu_path: Path, sample_start: int, sample_end: int) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    # Retain v2/v3 correction: Haifa/Ashdod quarterly rows are legacy-terminal quarterlies.
    q_legacy = (
...
def monthlyize_terminal_teu(terminal_tons: pd.DataFrame, q_term: pd.DataFrame, m_port: pd.DataFrame) -> pd.DataFrame:
    """
    Monthly TEU only for annual-anchor construction and schema compatibility.

    We still build monthly TEU as in earlier versions, but the v4 labor allocator
    itself does not depend on month-specific terminal tons or month-specific
    terminal activity. The new quarter-flat rule depends on quarterly shares.
    """
...
def build_quarter_grid(teu_monthly: pd.DataFrame) -> pd.DataFrame:
    ...
    # Monthly counts per quarter by terminal.
    counts = (
    ...
    # Opening-quarter active-month count.
    quarter_grid["quarter_end"] = quarter_grid["quarter"].map(quarter_end_month)
    quarter_grid["quarter_start"] = quarter_grid["quarter_end"] - 2
    quarter_grid["active_months_in_q"] = quarter_grid["months_in_q"]

    for terminal, open_month in OPEN_MONTH_BY_TERMINAL.items():
        open_year = open_month // 100
        open_m = open_month % 100
        q_open = quarter_from_month(open_m)

        mask = (
            quarter_grid["terminal"].eq(terminal) &
            quarter_grid["year"].eq(open_year) &
            quarter_grid["quarter"].eq(q_open)
        )
        # Active months = opening month through quarter end.
        active = quarter_grid.loc[mask, "quarter_end"] - open_m + 1
        quarter_grid.loc[mask, "active_months_in_q"] = active.astype(float)

        # For pre-opening quarters of entrant, activity should be zero anyway.
        pre_mask = (
            quarter_grid["terminal"].eq(terminal) &
            ((quarter_grid["year"] * 100 + quarter_grid["quarter_end"]) < open_month)
        )
...
def build_monthly_labor(teu_monthly: pd.DataFrame, annual_anchor: pd.DataFrame) -> pd.DataFrame:
    ...
    Month labor:
        - ordinary quarter: L_{i,m} = L_{i,q} / 3
        - opening quarter:  split evenly across active months only; pre-opening
          months remain zero

    This is intentionally blunt, but it is meant to be a more truthful monthly
    shadow of a quarterly-truth object.
    """
    ...
    # Fallback only when annual activity is zero but annual TEU is positive.
    qcount = (
    ...
    # Expand quarter labor to months.
    out = base.merge(
        quarter_grid[
            ["port", "terminal", "year", "quarter", "tons_port_q", "activity_i_q_raw", "activity_i_y",
    ...
    out["is_active_month"] = 1.0
    for terminal, open_month in OPEN_MONTH_BY_TERMINAL.items():
        mask_pre = out["terminal"].eq(terminal) & (out["month_index"] < open_month)
        out.loc[mask_pre, "is_active_month"] = 0.0
    ...
    out["labor_weight_source"] = "quarter_flat_with_opening_mask"
...
def build_schema_compatible_lproxy(monthly_labor: pd.DataFrame, base_lproxy_path: Optional[Path]) -> pd.DataFrame:
    keep = [
        ...
        "tons_port_q", "activity_i_q_raw", "activity_i_y", "w_i_q_in_year", "L_hours_i_q",
...
def build_meta(
    ...
    return {
        ...
        "formula_main": {
            ...
            "month_labor": "L_{i,m} = L_{i,q} / active_months_in_q for active months; 0 for masked pre-opening months",
        },
        "fixes_relative_to_v3": [
            "quarter-truth allocator instead of month-specific share-times-port-tons",
            "flat within quarter to reduce spurious monthly wiggles",
            "opening-quarter split only across active months",
        ],
        "options": {
...
def main():
    ap = argparse.ArgumentParser(description="Build common-rule monthly labor proxy v4 in Data/L_proxy/common_rule/")
    ...
    outputs = {
        "terminal_month_labor": out_dir / "labor_hours_monthly_terminal_commonrule_v4.tsv",
        "port_month_labor": out_dir / "labor_hours_monthly_port_commonrule_v4.tsv",
        "annual_anchor": out_dir / "labor_anchor_terminal_year_commonrule_v4.tsv",
        "lproxy": out_dir / "L_Proxy_commonrule_v4.tsv",
        "qa": out_dir / "lproxy_commonrule_v4_qa.tsv",
        "anchor_compare": out_dir / "lproxy_commonrule_v4_old_new_anchor_compare.tsv",
        "meta": out_dir / "_meta_l_proxy_commonrule_v4.json",
    }
