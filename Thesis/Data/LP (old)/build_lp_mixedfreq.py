...
"""
Robust LP panel builder (error-proof with validation & schema normalization).

Granularity policy:
- Ports: always monthly (M), using monthly TEU when available and quarterly-TEU fallback for w post-reform.
- Terminals: monthly source-of-truth, plus an analysis view where terminals switch to quarterly after a cutover.
...
Optional config:
  - Data/LP/columns_map.json    # header mapping per file (canonical -> actual header)
  - Data/LP/cutover.yaml        # per-port YYYY-MM cutover (terminals -> quarterly)

CLI:
  --validate-only        Only run validation & exit 0/1 (friendly messages, no stacktrace)
  --columns_map          Path to columns_map.json (optional)
  --cutover_yaml         Path to cutover.yaml (optional)
  --cutover              "Haifa:2021-09,Ashdod:2022-07" (fallback if YAML missing)
  --base_dir, --l_proxy, --tons, --teu_mq, --out_dir, --winsor_*
...
def _norm_port(s: str) -> str:
    ...
    # Simplify common variants to canonical names
    low = s2.lower()
...
'''
def _read_cutover_yaml(path: Optional[str]) -> Dict[str,str]:
    if not path or not os.path.exists(path):
        return {}
    try:
        if yaml is not None:
            with open(path, "r", encoding="utf-8") as f:
                ...
                if not isinstance(obj, dict):
                    raise ValidationError("cutover.yaml must map port -> 'YYYY-MM'.")
                return {str(k): str(v) for k,v in obj.items()}
        # Fallback: parse simple "Port: YYYY-MM" lines
        m: Dict[str,str] = {}
        with open(path, "r", encoding="utf-8") as f:
            for line in f:
                if ":" in line:
        ...
        raise ValidationError(f"Failed to read cutover.yaml: {e}")
'''
...
def load_inputs(args) -> Inputs:
    ...
    # Cutover mapping
    cut_map = {}
    #yaml_map = _read_cutover_yaml(args.cutover_yaml) if args.cutover_yaml else {}
    #if yaml_map:
    #    cut_map.update(yaml_map)
    if args.cutover:
...
def _apply_header_map(df: pd.DataFrame, file_basename: str, columns_map: Dict[str, Dict[str, str]]) -> pd.DataFrame:
    # columns_map[file_basename] maps canonical -> actual header in the file
    ...
    for canonical, actual in mp.items():
        # If the actual exists, rename it to canonical
...
def load_L_proxy(path: str, columns_map: Dict[str, Dict[str,str]]) -> pd.DataFrame:
    ...
    # Required canonical names; we'll try to infer if missing
...
def compute_w(tons_pm: pd.DataFrame, teu_pm: pd.DataFrame, teu_pq: pd.DataFrame,
    ...
    w_m["w_src_monthly"] = pd.Series(
        np.where(w_m["tons_per_teu"].notna(), "monthly", None),
        index=w_m.index,
        dtype="object"
    )
    ...
    if teu_pq.empty:
        ...
        # keep dtype purely object to avoid NumPy promotion issues
        ...
        w_qm = map_q_to_m.merge(rq[["port","year","quarter","w_p_q"]],
                                on=["port","year","quarter"], how="left")
        w_qm = w_qm.rename(columns={"w_p_q":"w_from_q"})
        w_qm["w_src_quarterly"] = pd.Series(
            np.where(w_qm["w_from_q"].notna(), "quarterly", None),
            index=w_qm.index,
            dtype="object"
        )
...
def main():
    ...
    ap.add_argument("--cutover_yaml", default=None, help="Path to cutover.yaml (optional).")
    ...
    try:
        ...
        if not ok:
            # Exit cleanly without stacktrace
            sys.exit(1)
        ...
        # Catch-all with friendly message (avoid full pandas trace)
