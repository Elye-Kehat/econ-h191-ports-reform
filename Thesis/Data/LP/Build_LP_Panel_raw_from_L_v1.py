...
"""
Build_LP_Panel_raw_from_L_v1.py
================================

Purpose
-------
Build an alternative labor-productivity panel using the identity-style definition

    LP_raw = throughput / labor

rather than the current mix-adjusted Stage 4 construction.

This file is meant to be the clean "try it" implementation discussed in chat:
- keep Stage 1 (tons), Stage 2 (TEU), and Stage 3 (L_Proxy harmonization)
- replace the current Stage 4 / Stage 5 LP construction with a simpler raw LP build

Why this file exists
--------------------
The current LP pipeline is split across five stages:
- Stage 1: tons
- Stage 2: TEU
- Stage 3: L_Proxy harmonization
- Stage 4: mix-adjusted LP construction
- Stage 5: stack the six LP series

If you want to try the simpler outcome

    LP_raw = Y / L

the cleanest change is NOT to rewrite all five stages. Instead:
- keep S1/S2/S3, because they still produce the right ingredients
- replace the old S4/S5 logic with one direct builder that computes raw LP
  from those ingredients and writes a final LP panel

What this script builds
-----------------------
1) Monthly port LP for the two port series:
       LP_{p,m} = TEU_{p,m} / L_{p,m}

2) Quarterly terminal LP for the four terminal series:
       LP_{i,q} = TEU_{i,q} / L_{i,q}

where quarterly labor is built by summing monthly terminal labor inside each quarter:

       L_{i,q} = sum_{m in q} L_{i,m}

Design choices
--------------
- We intentionally keep the same support structure as the current LP pipeline:
  * monthly port series
  * quarterly terminal series

- For monthly port LP, we use:
    S2_port_month_teu
    S3_port_month_labor
    S1_port_month_tons (for metadata only)

- For quarterly terminal LP, we use:
    S2_terminal_quarter_teu
    S3_lproxy_clean aggregated to quarter
    S1_terminal_quarter_tons if available (for metadata only)

- We write the six component files using the standard filenames expected by the
  current stack convention, but inside a separate output folder. That way you
  can inspect and compare them before promoting anything into Data/LP/.

Important limitation
--------------------
This script is intentionally simple. It drops the explicit cargo-mix adjustment
from the current maintained LP pipeline. So this is not a "small tweak" to the
old LP definition. It is an alternative LP outcome definition.

Outputs
-------
Inside --out it writes:
...
def stack_panel(monthly_port: pd.DataFrame, quarterly_term: pd.DataFrame) -> pd.DataFrame:
    hm = monthly_port[monthly_port["port"] == "Haifa"].copy()
    am = monthly_port[monthly_port["port"] == "Ashdod"].copy()
...
def main():
    ...
    ap.add_argument("--out", default="Data/LP/raw_from_l_v1")
    ...
    meta = {
        "builder": "Build_LP_Panel_raw_from_L_v1.py",
        "definition": {
        ...
        "inputs": {
            ...
            "s3_lproxy_clean": args.s3_lproxy_clean,
            "s3_port_month_labor": args.s3_port_month_labor,
        },
        "windows": {
        ...
        "outputs": {
            "LP_Haifa_port_month": os.path.join(args.out, "LP_Haifa_port_month.tsv"),
            "LP_Ashdod_port_month": os.path.join(args.out, "LP_Ashdod_port_month.tsv"),
            "LP_Haifa_Legacy_quarter": os.path.join(args.out, "LP_Haifa_Legacy_quarter.tsv"),
            "LP_Haifa_SIPG_quarter": os.path.join(args.out, "LP_Haifa_SIPG_quarter.tsv"),
            "LP_Ashdod_Legacy_quarter": os.path.join(args.out, "LP_Ashdod_Legacy_quarter.tsv"),
            "LP_Ashdod_HCT_quarter": os.path.join(args.out, "LP_Ashdod_HCT_quarter.tsv"),
            "LP_Panel": os.path.join(args.out, "LP_Panel.tsv"),
            "LP_raw_qa": os.path.join(args.out, "LP_raw_qa.tsv"),
        },
        "notes": [
            "This builder keeps S1/S2/S3 and replaces the mix-adjusted S4/S5 logic with raw TEU over labor.",
            "w is set to 1.0 as a compatibility placeholder; no separate mix factor is estimated here.",
            "Pi is left missing because this alternative LP definition does not construct a separate intrinsic productivity baseline.",
        ],
    }
    ...
    print("[LP_raw] Wrote raw throughput-over-labor LP artifacts to", args.out)
