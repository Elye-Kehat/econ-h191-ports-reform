...
"""
Build_LP_Panel_raw_from_L_v5_tonsonly.py
=======================================

Purpose
-------
Refine the v4 tons-only LP builder so the monthly port LP branch lines up with the
actual competition design used by Model 1A.

Why v5 exists
-------------
v4 fixed the broken monthly port labor merge, which restored non-missing monthly
port LP rows. But it still used one common monthly cutoff for both ports:

    monthly_end = 202110

That is not aligned with the empirical design.

For Model 1A competition, the pre-reform port LP object should end at different
calendar dates by port:

    Haifa port monthly LP   -> keep through 2021-08 (pre 2021Q3 entry)
    Ashdod port monthly LP  -> keep through 2022-09 (pre 2022Q4 entry)

If we keep one shared cutoff, then:
- Haifa port LP extends too far into the post-entry period, and
- Ashdod port LP stops too early, which weakens the Ashdod incumbent splice.

Core fix in v5
--------------
1) Keep the v4 merge fix:
   merge monthly port TEU, labor, and tons by (port, year, month)

2) Replace the single shared monthly cutoff with port-specific cutoffs:
       Haifa  <= 202108
       Ashdod <= 202209

3) Add QA checks that verify:
   - monthly port LP is not entirely missing for either port
   - no Haifa port month survives past the Haifa cutoff
   - no Ashdod port month survives past the Ashdod cutoff
...
def build_quarterly_terminal_raw(args) -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    if args.s1_terminal_quarter_tons and Path(args.s1_terminal_quarter_tons).exists():
        ...
        if {"port", "terminal", "year", "quarter", "tons_i_q"}.issubset(set(t_tq.columns)):
            # Keep the quarterly tons merge simple for now. It is not the current blocker for Model 1A,
            # but we retain it for bookkeeping and later diagnostics.
            out = out.merge(
...
def main():
    ...
    parser.add_argument("--out", default=str(thesis_root / "Data" / "LP" / "raw_from_l_v5_tonsonly"))

    # Keep one shared monthly start but use port-specific monthly ends that line up with the design.
    ...
    meta = {
        "builder": "Build_LP_Panel_raw_from_L_v5_tonsonly.py",
        "definition": {
        ...
        "why_v5": [
            "keeps the v4 monthly port labor merge fix by joining on (port, year, month)",
            "replaces the shared monthly cutoff with port-specific cutoffs aligned to the competition design",
            "keeps the quarterly terminal LP branch unchanged",
            "adds hard QA checks that the port monthly branch respects each port's cutoff",
        ],
        "inputs": {
    ...
    meta_path = os.path.join(args.out, "_meta_lp_raw_v5_tonsonly.json")
    ...
    print("[LP_raw_v5_tonsonly] Wrote raw throughput-over-labor LP artifacts to", args.out)
