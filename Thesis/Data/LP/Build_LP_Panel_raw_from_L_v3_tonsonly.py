...
"""
Build_LP_Panel_raw_from_L_v3_tonsonly.py
=======================================

Purpose
-------
Rebuild the LP panel on top of the new tons-only labor proxy.

Conceptual role in the pipeline
-------------------------------
The main conceptual rewrite is in the labor builder. This LP builder is the downstream
rebuild that turns the new labor proxy into the mixed-frequency LP panel used by
Model 1A.

Current LP definition retained
------------------------------
We keep the current active LP definition from the raw-from-L branch:
...
-----------------------
The point of this file is not to invent a new LP concept. It is to regenerate the LP
panel after replacing the labor proxy with the v6 tons-only labor build.

Relationship to Model 1A v7
---------------------------
Model_1A_v7_quarterly_windows.py uses:
- pre-reform monthly port LP, collapsed to quarter means of log(LP)
- post-reform terminal LP at native quarterly frequency

So this builder preserves exactly that mixed-frequency panel structure while swapping in
new labor inputs.

What changed relative to raw_from_L_v2
--------------------------------------
1) The labor input now comes from the v6 tons-only L_Proxy.
2) We no longer need a separate S3 harmonization file if the new L_Proxy is already
   clean and schema-compatible.
3) The rest of the LP formula remains intentionally simple and unchanged.

Primary outputs
---------------
By default, outputs are written to:
    Data/LP/raw_from_l_v3_tonsonly/
...
so Model 1A v7 can read the new LP panel through the standard canonical path.
"""
...
def build_monthly_port_raw(args) -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    _require_columns(lproxy, ["port", "year", "month", "month_index", "L_hours_i_m"], "L_Proxy_v6")

    labor_p_m = (
        lproxy.groupby(["port", "year", "month", "month_index"], as_index=False)["L_hours_i_m"]
        .sum(min_count=1)
    ...
    out = (
        teu_m.merge(labor_p_m[["port", "year", "month", "month_index", "L_hours_port_m"]], on=["port", "year", "month", "month_index"], how="left")
        .merge(tons_m[["port", "year", "month", "month_index", "quarter", "tons_port_m", "tons_source"]], on=["port", "year", "month", "month_index"], how="left")
    )
...
def build_quarterly_terminal_raw(args) -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    if args.s1_terminal_quarter_tons and Path(args.s1_terminal_quarter_tons).exists():
        ...
        if {"port", "terminal", "year", "quarter", "tons_i_q"}.issubset(set(t_tq.columns)):
            out = out.merge(t_tq[["port", "terminal", "year", "quarter", "tons_i_q"]], on=["port", "terminal", "year", "quarter"], how="left")
        else:
            out["tons_i_q"] = np.nan
    else:
        out["tons_i_q"] = np.nan
...
def main():
    ...
    parser.add_argument("--out", default=str(thesis_root / "Data" / "LP" / "raw_from_l_v3_tonsonly"))
    ...
    meta = {
        "builder": "Build_LP_Panel_raw_from_L_v3_tonsonly.py",
        "definition": {
        ...
        "why_v3": [
            "rebuilds LP after replacing the labor proxy with tons-only within-year labor allocation",
            "keeps the active raw throughput-over-labor LP definition unchanged",
            "preserves the mixed-frequency LP panel schema expected by Model 1A",
        ],
        "inputs": {
    ...
    meta_path = os.path.join(args.out, "_meta_lp_raw_v3_tonsonly.json")
    ...
    print("[LP_raw_v3_tonsonly] Wrote raw throughput-over-labor LP artifacts to", args.out)
