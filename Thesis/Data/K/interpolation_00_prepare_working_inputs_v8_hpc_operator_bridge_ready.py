#!/usr/bin/env python3
"""
interpolation_00_prepare_working_inputs_v8_hpc_operator_bridge_ready.py
...
New in v6
---------
Certain project rows in the raw dated-events file are informative about timing, but are not
appropriate to treat as additive booked monthly capital flows in the engine. The main current
case is the HPC Kishon West cranes project headline amount. The evidence reviewed for this
project suggests that it is a broader multi-stage project amount rather than a clean single-month
booked service-entry flow. Because annual investment totals are authoritative in this pipeline,
adding that headline amount on top of the annual pools would mechanically double count it.

So v4:
1. keeps the right-of-use exclusion logic from v3,
2. keeps the broader SIPG service-entry alias capture,
3. keeps productive-capital-consistent annual totals,
4. keeps raw stock anchors preserved but explicitly labeled as unadjusted for productive scope,
5. reclassifies specific problematic dated events from “additive monthly event” to
   “non-additive project milestone” in interpolation_00, before the monthly engine runs.
6. patches SIPG so that the productive-capital series is driven by Transfers to PPE
   rather than by CIP additions.
7. rebuilds SIPG rough stock anchors from cumulative transfers only in years where that
   cumulative measure is informative for productive capital, and leaves later flat
   cumulative-transfer years unanchored rather than forcing the monthly engine to undo
   depreciation to hit a gross cumulative amount.

This is done in interpolation_00 rather than interpolation_01 because the implementation
strategy explicitly places economically sensitive filtering and transformation decisions in the
preprocessing stage, before the monthly recursion begins.

Bridge-ready note
-----------------
This file does not itself change HPC monthly timing. That is handled in interpolation_01.
But Step 00 is now explicitly bridge-ready: it preserves the stable productive-capital inputs,
and Step 01 is expected to apply a smooth bridge with HPC-specific operator-style linear
backloaded timing.
"""
...
def main() -> None:
    ...
    manifest = {
        ...
        "decisions_applied": {
            ...
            "step01_hpc_operator_linear_backloaded_timing_expected": True,
        },
    }
