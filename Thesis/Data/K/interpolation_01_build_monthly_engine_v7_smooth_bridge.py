#!/usr/bin/env python3
"""
interpolation_01_build_monthly_engine_v7_smooth_bridge.py
...
3. The bridge is intentionally conservative: it stays as close as possible to the previous strategy by
   using the same entity-year background month-share schedules to distribute the residual needed to
   land on the next hard anchor.
4. Missing hard-anchor years are still propagated; if the next observed stock anchor is later, the bridge
   spans the full open block.
5. SIPG remains unchanged conceptually: its cumulative-transfer pseudo-anchors are still diagnostic only.
