#!/usr/bin/env python3
"""
interpolation_02_finalize_outputs_and_qc_v9_hpc_operator_linear_backloaded.py
...
-----------------------
1. Keeps the five main visualization outputs.
2. Fills the final entity-month grid explicitly so each final series has every month from
   2018-01 through 2024-12, with zeros where an entity has no productive capital yet.
3. Uses the revised Step 01 bridge diagnostics table as the authoritative source for hard-anchor fit.
4. Prefers the baseline monthly investment ledger, when available, for the annual-flow identity
   diagnostic so the warning is not mechanically driven by smooth bridge adjustments.
5. Writes two QC detail tables for easier diagnosis.
6. Records the HPC operator-style linear backloaded timing rule in the build manifest so the
   final-series outputs can be linked back to the revised Step 01 timing choice.
"""
...
def load_required(prepared_dir: Path) -> Dict[str, pd.DataFrame]:
    ...
    optional = {
        "monthly_flows_baseline": prepared_dir / "interpolation_01_monthly_class_investment_baseline.tsv",
        "entity_month_capital": prepared_dir / "interpolation_01_monthly_entity_capital.tsv",
        "bridge_adjustments": prepared_dir / "interpolation_01_bridge_adjustments.tsv",
    }
    for k, path in optional.items():
        out[k] = read_tsv(path) if path.exists() else pd.DataFrame()
    return out
...
def complete_entity_month_grid(entity_month: pd.DataFrame) -> pd.DataFrame:
    months = month_range(SAMPLE_START, SAMPLE_END)
    full_index = pd.MultiIndex.from_product([FINAL_ENTITIES, months], names=["entity", "month"])
    out = entity_month.set_index(["entity", "month"]).reindex(full_index).reset_index()
...
def build_long_and_wide(entity_month: pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame]:
    long_df = complete_entity_month_grid(entity_month).sort_values(["entity", "month"]).reset_index(drop=True)
...
def qc_annual_flow_identity(detail: pd.DataFrame) -> Dict[str, object]:
    ...
    return {
        ...
        "notes": (
            ...
            "Mismatches can arise when annual totals are broader than productive engine inputs; when a baseline monthly ledger is available, this check is run on that baseline before smooth bridge adjustments are added"
        ),
    }
...
def build_qc(anchors, monthly_flows_for_qc, long_df, wide_df, recon):
    annual_detail = annual_flow_identity_detail(anchors, monthly_flows_for_qc)
...
def main() -> None:
    ...
    monthly_flows_baseline = data["monthly_flows_baseline"]
    class_capital = data["class_capital"]
    recon = data["recon"]
    entity_month_from_step01 = data["entity_month_capital"]

    if not entity_month_from_step01.empty:
        entity_month = entity_month_from_step01.copy()
        entity_month["K_productive_kNIS"] = as_num(entity_month["end_stock_total_kNIS"]).fillna(0.0)
        entity_month = entity_month[["entity", "month", "K_productive_kNIS"]]
    else:
    ...
    monthly_flows_for_qc = monthly_flows_baseline if not monthly_flows_baseline.empty else monthly_flows
    qc_summary, annual_detail, anchor_detail = build_qc(anchors, monthly_flows_for_qc, long_df, wide_df, recon)
    ...
    manifest = {
        ...
        "qc_design_notes": {
            "annual_flow_identity_prefers_baseline_step01_monthly_flows_when_available": True,
            "anchor_fit_uses_step01_bridge_diagnostics_table": True,
            "final_entity_month_grid_is_completed_and_zero_filled": True,
        },
    }
