...
def main() -> None:
    print("=== Model_2_to_tables: starting ===")
    ...
    df_term = df_term.replace([np.inf, -np.inf], np.nan).dropna(subset=["log_LP", "log_KL", "t_index"])
    df_clus = df_clus.replace([np.inf, -np.inf], np.nan).dropna(subset=["log_LP", "log_KL", "t_index"])

    DELTAS = [0.04, 0.06, 0.08]
    ENTITIES = ["Haifa--Legacy", "Haifa--Bayport", "Pooled"]
    ...
    def cell(entity: str, delta: float) -> Tuple[str, str, str, str]:
        ...
        r = out[(out["entity"] == key) & (out["delta"] == delta)]
        if r.empty:
            return "", "", "", ""
        r = r.iloc[0]
...
""".strip("\n").replace("{COEF_ROW}", coef_row).replace("{SE_ROW}", se_row).replace("{N_ROW}", n_row).replace("{R2_ROW}", r2_row)
    ...
    def cell_c(delta: float) -> Tuple[str, str, str, str]:
        r = out_c[out_c["delta"] == delta].iloc[0]
        coef = fmt(r["coef_log_KL"], 2) + stars(r["p_log_KL"])
        se = f"({fmt(r['se_log_KL'], 2)})"
        n = str(int(r["N"]))
        r2 = fmt(r["R2"], 3)
