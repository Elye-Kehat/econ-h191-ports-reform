...
-----------------------
The v7 quarterly shell improved frequency discipline, but the headline Haifa
privatization comparison still used a within-Haifa Legacy-versus-Bayport design.
That object is structurally contaminated by within-port substitution and Bayport
ramp-up.

This v8 rewrite keeps the quarterly-only estimation framework but changes the
privatization headline object to:

    treated: Haifa aggregate port
    control: Ashdod aggregate port

To support that pivot, v8 also reads direct quarterly aggregate-port LP rows from
LP_Panel.tsv and carries them through as continuous aggregate analysis units.

Main design choices
-------------------
1) Quarterly only
   Post-reform terminal LP is genuinely quarterly, and the aggregate-port pivot also
   relies on quarterly objects. We therefore keep the quarterly-only shell.

2) Preserve the v7 regression engine
   Event bins, summary windows, baseline FE, and port-trend shells stay the same.
   The main change is in the analysis units and the reform design list.

3) Continuous aggregate analysis units
   The LP builder now writes Haifa_port_Q and Ashdod_port_Q. Combined with the
   existing monthly port rows, this file constructs continuous quarterly aggregate-port
   series and exposes them as:

       haifa_port_agg
       ashdod_port_agg

4) Privatization pivot
   The preferred privatization object is now:

       Haifa-Aggregate vs Ashdod-Aggregate

   Legacy-vs-Bayport privatization objects are retained only as diagnostics under a
   separate table group.
"""
...
TARGET_TO_UNIT = {
    ...
    "Haifa-Aggregate": "haifa_port_agg",
    "Ashdod-Aggregate": "ashdod_port_agg",
}
...
def target_unit_for_design(d: ReformDesign) -> str:
    if d.target not in TARGET_TO_UNIT:
        raise KeyError(f"Unknown target mapping requested for target={d.target}")
...
def main() -> None:
    ...
    parser.add_argument(
        "--out",
        default="Design/Output (new)/Model_1A_v8",
    ...
    print(f"[Model_1A_v8] Using LP panel: {lp_path}")
    print(f"[Model_1A_v8] LP resolution mode: {lp_resolution}")
    print("[Model_1A_v8] Quarterly-only run: no monthly shell will be estimated.")

    results = run_quarterly_only(lp_path, outdir)

    manifest = {
        "script": "Model_1A_v8.py",
    ...
    with open(outdir / "model1a_v8_manifest.json", "w", encoding="utf-8") as f:
        json.dump(manifest, f, indent=2)

    print(f"[Model_1A_v8] Wrote outputs to: {outdir}")
