...
annotates the aggregated outputs from Model_1A.py.

NOTE (v2):
  A derived "post_yr2" window (m in [13,24]) used to be added from
  dynamic betas. Under the updated, truncated NYT windows (e.g., Haifa
  competition entry ending 10-2022), that constructed window is not
  meaningful. It is disabled by default and can be re-enabled later.
"""
...
# Options
# --------------------------------------------------------------------
# Under truncated NYT windows (e.g., Haifa-comp ends 10-2022), m in [13,24]
# is not a meaningful "year 2" window. Keep disabled unless windows expand.
ENABLE_DERIVED_POST_YR2 = False
...
# --------------------------------------------------------------------
# post_yr2 window constructor (m in [13,24])  [DISABLED BY DEFAULT IN v2]
# --------------------------------------------------------------------
...
def add_post_year2_windows(win: pd.DataFrame) -> pd.DataFrame:
    ...
    NOTE (v2): This function remains available, but is disabled by default.
    """
...
def build_windows_for_tables() -> pd.DataFrame:
    ...
    Take model1a_lp_window_betas_all.tsv and:
      ...
      * (optional) add a derived "post_yr2" window m∈[13,24] (disabled by default),
    ...
    # Optionally add derived m∈[13,24] "post_yr2" window (disabled by default)
    if ENABLE_DERIVED_POST_YR2:
...
def main() -> None:
    ...
    print("=== Model_1A_to_tables(v2): done ===")
