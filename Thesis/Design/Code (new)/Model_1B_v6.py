...
# Model_1B_v6.py
#
# Haifa-only K/L extension, v6.
#
# What changed relative to v5:
#   1) Reform-specific dynamic support:
#        competition   m in [-12, 24]
#        privatization m in [-12, 23]
#      No useless tail bins.
#
#   2) Two estimator families in one script:
#        A. exact-month dynamic event study      -> mainly for plots / diagnostics
#        B. relaxed binned event study           -> main table estimates
#
#   3) Overlap controls:
#        competition   regressions include post_priv
#        privatization regressions include post_comp
#
#   4) Two specifications for each family:
#        baseline    = event terms + overlap control
#        ctrl_trend  = event terms + overlap control + t_linear + covid_shock + war_shock
#
#   5) Pretrend handling:
#        - simple pre summary: average m in [-12,-2]
#        - joint pretrend test on the two lead bins [-12,-7] and [-6,-2] for the binned family
#
# Output folder:
#   Design/Output (new)/Model_1B_v6/
# =============================================================================
...
def clear_outputs(output_dir: Path) -> None:
    ...
    for pattern in ["model1b_*.tsv", "model1b_*.json"]:
        for path in output_dir.glob(pattern):
            ...
            removed += 1
    print(f"Cleared {removed} old Model 1B v6 output files from: {output_dir}")
...
def build_formula(family: str, reform: str, spec_name: str) -> str:
    overlap = REFORMS[reform]["overlap_control"]
    if family == "exact":
...
def exact_window_summary(result, sample: pd.DataFrame, series_id: str, reform: str, spec_name: str) -> pd.DataFrame:
    rows = []
    sample_months = set(sample["event_time"].astype(int).tolist())
    for window_key, a_req, b_req, label in REFORMS[reform]["windows"]:
        used_months = [m for m in range(a_req, b_req + 1) if m != -1 and m in sample_months]
        weights = {exact_param_name(str(m)): 1.0 / len(used_months) for m in used_months if m != -1} if used_months else {}
...
def exact_pretrend_test(result, sample: pd.DataFrame, series_id: str, reform: str, spec_name: str) -> pd.DataFrame:
    observed = set(sample["event_time"].astype(int).tolist())
    lead_names = [exact_param_name(str(m)) for m in range(-12, -1) if m != -1 and m in observed]
    f_val, p_val, df_num, df_denom = safe_wald_test(result, lead_names)
    lead_months = [m for m in range(-12, -1) if m != -1 and m in observed]
    return pd.DataFrame([{
        "family": "exact",
        "series_id": series_id,
        "series_display": SERIES_DISPLAY[series_id],
        ...
        "reform_display": REFORMS[reform]["display"],
        "spec_name": spec_name,
        "test_name": "joint_exact_leads",
        "n_terms_tested": int(len(lead_names)),
        "lead_min": int(min(lead_months)) if lead_months else np.nan,
        "lead_max": int(max(lead_months)) if lead_months else np.nan,
        "f_stat": f_val,
        "pvalue": p_val,
        "df_num": df_num,
        "df_denom": df_denom,
...
def binned_window_summary(result, sample: pd.DataFrame, series_id: str, reform: str, spec_name: str) -> pd.DataFrame:
    ...
    months_map = {
        "avg_pre": [m for m in sample["event_time"].astype(int).tolist() if -12 <= m <= -2],
        "post_1_6": [m for m in sample["event_time"].astype(int).tolist() if 1 <= m <= 6],
        "post_7_12": [m for m in sample["event_time"].astype(int).tolist() if 7 <= m <= 12],
        "post_13_tail": [m for m in sample["event_time"].astype(int).tolist() if m >= 13],
        "post_full": [m for m in sample["event_time"].astype(int).tolist() if m >= 1],
    }
    ...
    for window_key, a_req, b_req, label in REFORMS[reform]["windows"]:
        ...
        weights = {k: v / total for k, v in kept.items()} if total > 0 else {}
        beta, se, pvalue = linear_combo_from_params(result, weights)
...
def binned_pretrend_test(result, sample: pd.DataFrame, series_id: str, reform: str, spec_name: str) -> pd.DataFrame:
    tokens = []
    if (sample["binned_token"] == "pre_12_7").any():
        tokens.append(binned_param_name("pre_12_7"))
    if (sample["binned_token"] == "pre_6_2").any():
...
def build_table_cells(windows_df: pd.DataFrame, pre_df: pd.DataFrame, series_keep: List[str], table_group: str) -> pd.DataFrame:
    ...
    # add pretrend p row + N + R2 summary from same spec/reform/series
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Model 1B v6: exact + binned Haifa-only K/L dynamics")
    ...
    output_dir = Path(args.outdir) if args.outdir else thesis_root / "Design" / "Output (new)" / "Model_1B_v6"
    ...
    exact_windows_all: List[pd.DataFrame] = []
    exact_pre_all: List[pd.DataFrame] = []
    ...
    for series_id in ALL_SERIES:
        ...
        for reform in ["competition", "privatization"]:
            ...
            for spec_name in ["baseline", "ctrl_trend"]:
                ...
                exact_windows_all.append(exact_window_summary(result, sample, series_id, reform, spec_name))
                exact_pre_all.append(exact_pretrend_test(result, sample, series_id, reform, spec_name))
    ...
    exact_windows = pd.concat(exact_windows_all, ignore_index=True) if exact_windows_all else pd.DataFrame()
    exact_pre = pd.concat(exact_pre_all, ignore_index=True) if exact_pre_all else pd.DataFrame()
    ...
    paths = {
        ...
        "exact_windows": output_dir / "model1b_exact_window_betas_summary.tsv",
        "exact_pretrend": output_dir / "model1b_exact_pretrend_tests.tsv",
    ...
    exact_windows.to_csv(paths["exact_windows"], sep="\t", index=False)
    exact_pre.to_csv(paths["exact_pretrend"], sep="\t", index=False)
    ...
    manifest = {
        ...
        "families": {
            "exact": "exact-month event-study on reform-specific support",
    ...
    print(f"Wrote exact windows       : {paths['exact_windows']}")
    print(f"Wrote exact pretrends     : {paths['exact_pretrend']}")
