...
def aggregate_series_available(df: pd.DataFrame, port_name: str) -> bool:
    if port_name == "Haifa":
        candidates = {"haifa_aggregate_port", "haifa_aggregate", "Haifa aggregate"}
    else:
        candidates = {"ashdod_aggregate_port", "ashdod_aggregate", "Ashdod aggregate"}
...
def get_series_id_for_label(df: pd.DataFrame, label: str) -> str:
    if label not in LABEL_FILTERS:
        raise KeyError(f"Label {label!r} not found in LABEL_FILTERS.")
