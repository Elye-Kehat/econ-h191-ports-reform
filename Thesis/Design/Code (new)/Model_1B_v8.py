...
# Model_1B_v8.py
#
# Haifa-only K/L extension, v8.
#
# Main changes relative to v7:
#   1) Keep exact-month event study for plots / descriptive dynamics only.
#
#   2) Keep binned event-study as the inferential family, but add a second,
#      more parsimonious privatization variant so we can compare:
#         - detailed: pre_12_7, pre_6_2, m0, post_1_6, post_7_12, post_13_23
#         - compact : pre_12_2, m0, post_1_6, post_7_23
#
#      Competition remains on the detailed bins.
#
#   3) Simplify the privatization overlap control from three competition-age bins
#      to two bins:
#         - comp_4_12 (reference)
#         - comp_13_plus
#
#   4) Write two table-helper versions:
#         - detailed  : competition=detailed, privatization=detailed
#         - preferred : competition=detailed, privatization=compact
#
# Output folder:
#   Design/Output (new)/Model_1B_v8/
# =============================================================================
...
REFORMS: Dict[str, Dict[str, object]] = {
    ...
    "privatization": {
        ...
        "overlap_bins": [
            ...
            ("comp_13_plus", 13, None, "Competition overlap [13,+]"),
        ],
        ...
        "variants": {
            ...
            "compact": {
                "bin_defs": [
                    ("pre_12_2", -12, -2, "Lead bin [-12,-2]"),
                    ("ref_m1", -1, -1, "Reference month -1"),
                    ("m0", 0, 0, "Month 0"),
                    ("post_1_6", 1, 6, "Post bin [1,6]"),
                    ("post_7_tail", 7, 23, "Post bin [7,23]"),
                ],
                "windows": [
                    ("avg_pre", -12, -2, "Average pre, months -12 to -2"),
                    ("post_1_6", 1, 6, "Average post, months 1-6"),
                    ("post_7_tail", 7, 23, "Average post, months 7-23"),
                    ("post_full", 1, 23, "Average full post, months 1-23"),
                ],
                "pre_keys": ["pre_12_2"],
            },
...
def clear_outputs(output_dir: Path) -> None:
    ...
    for pattern in ["model1b_*.tsv", "model1b_*.json"]:
        for path in output_dir.glob(pattern):
            ...
            removed += 1
    print(f"Cleared {removed} old Model 1B v8 output files from: {output_dir}")
...
def get_variant_dict(reform: str, variant: str) -> Dict[str, object]:
    return REFORMS[reform]["variants"][variant]


def assign_binned_token(m: int, reform: str, variant: str) -> Optional[str]:
    for bin_key, a, b, _label in get_variant_dict(reform, variant)["bin_defs"]:
        if a <= m <= b:
            if bin_key == "ref_m1":
                return "-1"
            return str(bin_key)
...
def extract_binned_rows(result, sample: pd.DataFrame, series_id: str, reform: str, variant: str, spec_name: str) -> pd.DataFrame:
    rows = []
    variant_info = get_variant_dict(reform, variant)
    with warnings.catch_warnings():
        warnings.simplefilter("ignore")
        bse = result.bse
        pvalues = result.pvalues

    for bin_key, a, b, label in variant_info["bin_defs"]:
...
def _window_bin_weights(reform: str, variant: str, window_key: str, sample: pd.DataFrame) -> Dict[str, float]:
    variant_info = get_variant_dict(reform, variant)
    windows = {k: (a, b, label) for k, a, b, label in variant_info["windows"]}
    a_req, b_req, _label = windows[window_key]
    required_months = [m for m in range(a_req, b_req + 1) if m != -1]
    observed_months = set(sample["event_time"].astype(int).tolist())
    full_support = int(set(required_months).issubset(observed_months))
    if full_support == 0:
        return {}

    weights = {}
    for bin_key, a_bin, b_bin, _bin_label in variant_info["bin_defs"]:
        token = "-1" if bin_key == "ref_m1" else str(bin_key)
        if token in {"-1", "m0"}:
            continue
        bin_months = [m for m in range(a_bin, b_bin + 1) if m != -1]
        overlap = [m for m in bin_months if a_req <= m <= b_req]
        if len(overlap) == len(bin_months) and len(overlap) > 0:
            weights[binned_param_name(token)] = float(len(overlap))
...
def binned_window_summary(result, sample: pd.DataFrame, series_id: str, reform: str, variant: str, spec_name: str) -> pd.DataFrame:
    rows = []
    variant_info = get_variant_dict(reform, variant)
    obs_months = sorted(set(sample["event_time"].astype(int).tolist()))

    for window_key, a_req, b_req, label in variant_info["windows"]:
        required_months = [m for m in range(a_req, b_req + 1) if m != -1]
        full_support = int(set(required_months).issubset(set(obs_months)))
        weights = _window_bin_weights(reform, variant, window_key, sample)
        beta, se, pvalue = linear_combo_from_params(result, weights)
        if full_support == 0 or not weights:
            beta, se, pvalue = np.nan, np.nan, np.nan

        used_months = [m for m in obs_months if a_req <= m <= b_req and m != -1]
        rows.append({
            "family": "binned",
            "binned_variant": variant,
            "series_id": series_id,
...
def binned_pretrend_test(result, sample: pd.DataFrame, series_id: str, reform: str, variant: str, spec_name: str) -> pd.DataFrame:
    variant_info = get_variant_dict(reform, variant)
    obs_months = set(sample["event_time"].astype(int).tolist())

    tokens = []
    for pre_key in variant_info["pre_keys"]:
        match = [t for t in variant_info["bin_defs"] if t[0] == pre_key]
        if not match:
            continue
        _key, a, b, _label = match[0]
        required = set(range(a, b + 1))
        if required.issubset(obs_months):
            tokens.append(binned_param_name(pre_key))

    supported_full_test = int(len(tokens) == len(variant_info["pre_keys"]))
    if supported_full_test == 1:
        f_val, p_val, df_num, df_denom = safe_wald_test(result, tokens)
    else:
        f_val, p_val, df_num, df_denom = np.nan, np.nan, np.nan, np.nan
    ...
    for pre_key in variant_info["pre_keys"]:
        match = [t for t in variant_info["bin_defs"] if t[0] == pre_key]
        if match:
            lead_mins.append(match[0][1])
            lead_maxs.append(match[0][2])
    ...
    return pd.DataFrame([{
        ...
        "reform_display": REFORMS[reform]["display"],
        "spec_name": spec_name,
        "test_name": "lead_bin_test",
        "n_terms_tested": int(len(tokens)),
        "supported_full_test": supported_full_test,
        "lead_min": int(min(lead_mins)) if lead_mins else np.nan,
        "lead_max": int(max(lead_maxs)) if lead_maxs else np.nan,
        "f_stat": f_val,
        "pvalue": p_val,
        "df_num": df_num,
        "df_denom": df_denom,
...
def build_table_cells(
    ...
    pre_df: pd.DataFrame,
    series_keep: List[str],
    table_group: str,
    variant_map: Dict[str, str],
) -> pd.DataFrame:
    selected_windows = []
    selected_pre = []
    for reform, variant in variant_map.items():
        selected_windows.append(
            windows_df.loc[
                (windows_df["family"] == "binned")
                & (windows_df["series_id"].isin(series_keep))
                & (windows_df["reform"] == reform)
                & (windows_df["binned_variant"] == variant)
            ].copy()
        )
        selected_pre.append(
            pre_df.loc[
                (pre_df["family"] == "binned")
                & (pre_df["series_id"].isin(series_keep))
                & (pre_df["reform"] == reform)
                & (pre_df["binned_variant"] == variant)
            ].copy()
        )

    win = pd.concat(selected_windows, ignore_index=True) if selected_windows else pd.DataFrame()
    pre = pd.concat(selected_pre, ignore_index=True) if selected_pre else pd.DataFrame()
    if win.empty:
        return pd.DataFrame()

    reform_order = {"competition": 1, "privatization": 2}
    series_order = {"Haifa_Legacy_KL": 1, "Haifa_port_KL": 2, "Haifa_Bayport_KL": 1}
    ...
    for _, r in win.iterrows():
        ...
        rows.append({
            ...
            "binned_variant": r["binned_variant"],
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Model 1B v8: exact plot dynamics + binned inference with privatization variants")
    ...
    output_dir = Path(args.outdir) if args.outdir else thesis_root / "Design" / "Output (new)" / "Model_1B_v8"
    ...
    for series_id in ALL_SERIES:
        ...
        for reform in ["competition", "privatization"]:
            ...
            for spec_name in ["baseline", "ctrl_trend"]:
                ...
                exact_formula = build_formula("exact", reform, spec_name)
                exact_result = fit_hac_ols(exact_formula, sample, args.hac_lags)
                exact_rows_all.append(extract_exact_rows(exact_result, sample, series_id, reform, spec_name))

            variants = list(REFORMS[reform]["variants"].keys())
            for variant in variants:
                sample_variant = sample.copy()
                sample_variant["binned_token"] = sample_variant["event_time"].astype(int).map(lambda m: assign_binned_token(m, reform, variant))
                for spec_name in ["baseline", "ctrl_trend"]:
                    print(f"    Binned spec: {variant} / {spec_name}")
                    binned_formula = build_formula("binned", reform, spec_name)
                    binned_result = fit_hac_ols(binned_formula, sample_variant, args.hac_lags)
                    binned_rows_all.append(extract_binned_rows(binned_result, sample_variant, series_id, reform, variant, spec_name))
                    binned_windows_all.append(binned_window_summary(binned_result, sample_variant, series_id, reform, variant, spec_name))
                    binned_pre_all.append(binned_pretrend_test(binned_result, sample_variant, series_id, reform, variant, spec_name))
    ...
    plot_helper = exact_rows.loc[exact_rows["event_time"] != -1].copy().sort_values(["series_id", "reform", "spec_name", "event_time"])

    main_table_detailed = build_table_cells(binned_windows, binned_pre, MAIN_SERIES, "main_detailed", TABLE_VARIANTS["detailed"])
    main_table_preferred = build_table_cells(binned_windows, binned_pre, MAIN_SERIES, "main_preferred", TABLE_VARIANTS["preferred"])
    bayport_table_detailed = build_table_cells(binned_windows, binned_pre, SUPPLEMENTARY_SERIES, "bayport_detailed", TABLE_VARIANTS["detailed"])
    bayport_table_preferred = build_table_cells(binned_windows, binned_pre, SUPPLEMENTARY_SERIES, "bayport_preferred", TABLE_VARIANTS["preferred"])
    ...
    paths = {
        ...
        "main_table_detailed": output_dir / "model1b_table_cells_main_detailed.tsv",
        "main_table_preferred": output_dir / "model1b_table_cells_main_preferred.tsv",
        "bayport_table_detailed": output_dir / "model1b_table_cells_bayport_detailed.tsv",
        "bayport_table_preferred": output_dir / "model1b_table_cells_bayport_preferred.tsv",
    ...
    main_table_detailed.to_csv(paths["main_table_detailed"], sep="\t", index=False)
    main_table_preferred.to_csv(paths["main_table_preferred"], sep="\t", index=False)
    bayport_table_detailed.to_csv(paths["bayport_table_detailed"], sep="\t", index=False)
    bayport_table_preferred.to_csv(paths["bayport_table_preferred"], sep="\t", index=False)

    manifest = {
        "script": "Model_1B_v8.py",
        ...
        "families": {
            ...
            "binned": "binned event-study used for the main tables; includes privatization detailed vs compact variants",
        },
    ...
    print(f"Wrote main table detailed : {paths['main_table_detailed']}")
    print(f"Wrote main table preferred: {paths['main_table_preferred']}")
    print(f"Wrote Bayport detailed    : {paths['bayport_table_detailed']}")
    print(f"Wrote Bayport preferred   : {paths['bayport_table_preferred']}")
