...
def load_baseline_and_relaxed() -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    static_base = read_tsv(BASE_DIR / "model1b_kl_static_betas_all_twfe.tsv", BASE_REQUIRED_STATIC)
    win_base = read_tsv(BASE_DIR / "model1b_kl_window_betas_all.tsv", BASE_REQUIRED_WINDOWS)
    pre_base = read_tsv(BASE_DIR / "model1b_kl_pretrend_tests_all.tsv", BASE_REQUIRED_PRE)

    static_rel = read_tsv(RELAXED_DIR / "model1b_kl_static_betas_all_relaxed_twfe.tsv", BASE_REQUIRED_STATIC)
    win_rel = read_tsv(RELAXED_DIR / "model1b_kl_window_betas_all_relaxed.tsv", BASE_REQUIRED_WINDOWS)
    pre_rel = read_tsv(RELAXED_DIR / "model1b_kl_pretrend_tests_all_relaxed.tsv", BASE_REQUIRED_PRE)

    static_all = pd.concat([static_base, static_rel], ignore_index = True)
    win_all = pd.concat([win_base, win_rel], ignore_index = True)
    pre_all = pd.concat([pre_base, pre_rel], ignore_index = True)
